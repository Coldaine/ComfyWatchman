...
4. Add `ComfyClient` read-only probes using donor route coverage as the checklist.
5. Add environment discovery with Desktop, portable, manual clone, `comfy-cli`, `extra_model_paths.yaml`, and active server detection.
6. Split workflow reading by format and normalize into a dependency schema.
7. Produce a dry-run readiness/install plan before any downloads or installs.
8. Add an MCP facade with explicit ComfyWatchman tools.
9. Add queue/history/output execution tools behind approved plans.
10. Add optional curated workflow-as-tool support using `PARAM_*` or Pixelle-style annotations.
11. Add visualization and registry-backed node-pack knowledge generation.
