"""
Enhanced Utility Functions for ComfyFixerSmart

Improvements based on guideposts:
1. Unify constants and behaviors (one extensions set; consistent error handling)
2. Improve recall without sacrificing safety (scan filenames in inputs; allow Civitai search even with unknown extensions)
3. Small hardening (hash algo guard; remove dead code/imports)
"""
...
# Single source of truth for model-ish extensions we care about
...
def validate_and_sanitize_filename(filename: Any) -> tuple[bool, str, Optional[str]]:
    ...
    # Basic extension allow-list (still enforced for writing; search may be looser).
        ...
        if isinstance(inputs, dict):
            for v in inputs.values():
                if isinstance(v, str) and _is_model_filename(v):
                    parts = re.split(r"[\\/]+", v)
                    filename = parts[-1] if parts else v
                    models.append(
                        {
                            "filename": filename,
                            "type": determine_model_type(node_type),
...
def build_local_inventory(models_dir: PathLike, extensions: Optional[List[str]] = None) -> Set[str]:
    ...
    """
    exts = set(extensions) if extensions else set(MODEL_EXTENSIONS)
...
def civitai_get_model_by_id(model_id: int, api_key: Optional[str] = None, timeout: int = 30) -> Json:
    ...
    try:
        ...
        raise ValueError(f"Civitai network error for model {model_id}: {e}") from e
...
def civitai_get_version_by_hash(sha256: str, api_key: Optional[str] = None, timeout: int = 30) -> Optional[Json]:
    ...
    Returns the version JSON or None if not found or on 404.
    """
    _require_requests()
...
def civitai_search_by_filename(
    ...
    # Allow search even if extension is unknown; keep strictness for writing later.
    ok, safe_name, err = validate_and_sanitize_filename(filename)
    effective_name = safe_name or filename
            ...
            for f in ver.get("files", []) or []:
                if f.get("name") == effective_name:
                    ...
                    return {
                        ...
                        "download_url": dl,
                        "confidence": 100,
                        "type": _infer_type_from_filename(effective_name),
                    }
    ...
    # If we failed initial validation only because of ext, reflect that in reason.
    if not ok and err:
...
def _infer_type_from_filename(name: str) -> str:
    low = name.lower()
    if low.endswith((".safetensors", ".ckpt", ".pt", ".pth")):
        if "vae" in low:
            return "vae"
        if any(token in low for token in ("lora", "lycoris", "locon")):
            return "loras"
        return "checkpoints"
...
def quick_inspect(path: PathLike, do_hash: bool = False) -> Dict[str, Any]:
    ...
    size = p.stat().st_size
    ext = p.suffix.lower()
    fmt = ext.lstrip(".") if ext in MODEL_EXTENSIONS else "other"
