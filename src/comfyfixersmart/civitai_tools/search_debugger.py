...
class SearchDebugger:
    ...
    def debug_search(self, query: str, model_type: str = "LORA", nsfw: bool = True) -> Dict[str, Any]:
        """
    ...
    def _debug_query_search(self, query: str, model_type: str, nsfw: bool) -> Dict[str, Any]:
        ...
        return {
            ...
            'items_returned': len(response_json.get('items', [])) if isinstance(response_json, dict) else 0
        }
    ...
    def _analyze_results(self, debug_info: Dict[str, Any]) -> List[str]:
        ...
        if query_nsfw_result.get('items_returned', 0) == 0 and query_without_nsfw_result.get('items_returned', 0) == 0:
    ...
    def _generate_suggestions(self, debug_info: Dict[str, Any]) -> List[str]:
        ...
        if all(debug_info['results'][key].get('items_returned', 0) == 0 
        ...
        if tag_results:
            valid_tag_results = [tag_result for tag_result in tag_results 
                                if tag_result.get('items_returned', 0) > 0]
