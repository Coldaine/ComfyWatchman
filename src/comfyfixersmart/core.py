...
class WorkflowRun:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'errors': self.errors,
            'uncertain_models': self.uncertain_models,
        }
...
class ComfyFixerCore:
    ...
    def _search_missing_models(self, missing_models: List[Dict[str, Any]],
        ...
        if search_backends is None:
            search_backends = list(config.search.backend_order)
        ...
        uncertain_count = sum(1 for result in search_results if result.status == 'UNCERTAIN')
        self.current_run.models_resolved = resolved_count
        self.current_run.uncertain_models = uncertain_count
        self.current_run.search_results = [result.__dict__ for result in search_results]

        self.logger.info(f"Resolved {resolved_count}/{len(missing_models)} models")
        if uncertain_count:
            self.logger.info(f"{uncertain_count} models require manual review (UNCERTAIN)")
    ...
    def _log_completion_summary(self):
        ...
        if run.uncertain_models:
            self.logger.info(f"Models requiring review (UNCERTAIN): {run.uncertain_models}")
